<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Agregar Nuevo Servicio Médico') }}
        </h2>
    </x-slot>
        ...
        <div class="w-full md:w-8/12 lg:w-6/12">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6 space-y-4">
                <div class="flex justify-between items-center">
                    <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-200">Servicio Médico</h2>
    ...
    <div>
        ...
        <input type="text" class="w-full px-3 py-2 border rounded-md focus:outline-none focus:border-blue-500 placeholder-gray-500 @error('nombre') border-red-500 @enderror" id="nombre" name="nombre" value="{{ old('nombre') }}" placeholder="Ingrese el nombre del servicio" required>
        @error('nombre')
    ...
    <div>
        ...
        <textarea class="w-full px-3 py-2 border rounded-md focus:outline-none focus:border-blue-500 placeholder-gray-500 @error('descripcion') border-red-500 @enderror" id="descripcion" name="descripcion" placeholder="Ingrese la descripción del servicio">{{ old('descripcion') }}</textarea>
        @error('descripcion')
            <span class="text-red-500 mt-1">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div>
        ...
        <input type="number" class="w-full px-3 py-2 border rounded-md focus:outline-none focus:border-blue-500 placeholder-gray-500 @error('costo') border-red-500 @enderror" id="costo" name="costo" value="{{ old('costo') }}" placeholder="Ingrese el costo del servicio" required>
        @error('costo')
            <span class="text-red-500 mt-1">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div>
        <input type="submit" class="w-full bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded-md shadow-md cursor-pointer" value="Agregar Servicio Médico">
    </div>
</form>
